...
static ssize_t dev_attr_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
  int ret = -1;
  sprintf(buf, "%d\n", address_file);
  if(global_dev == NULL)
...
static struct kobj_attribute i2c_slave_attr =__ATTR(address_file, 0660, dev_attr_show, dev_attr_store);
...
static int __init bcm2835_i2c_slave_init(void)
{
  ...
  // Creates directory in /sys/kernel
  i2c_kobject = kobject_create_and_add("i2c_slave_dir", kernel_kobj);
  if(!i2c_kobject)
    return -ENOMEM;

  // Creates a single file attribute
  error = sysfs_create_file(i2c_kobject, &i2c_slave_attr.attr);
  if(error){
    pr_debug("failed to create the address_file in /sys/kernel/i2c_slave_dir\n");
  }
...
MODULE_AUTHOR("Pierre Magne <pierre.magne@enseirb-matmeca.fr>");
MODULE_AUTHOR("Anthony Duarte <anthony.duarte@enseirb-matmeca.fr>");
